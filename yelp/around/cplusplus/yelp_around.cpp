...
double              g_lat;
double              g_lng;
char const *        g_filter_category = NULL;
...
query_circle(aerospike * asp, double lat, double lng, double radius)
{
	...
	if (g_filter_category) {
		...
		as_arraylist_append_str(&args, g_filter_category);
		as_query_apply(&query, "filter_by_category", "apply_filter",
					   (as_list *) &args);
	}
...
{
	string path = "filter_by_category.lua";
...
usage(int & argc, char ** & argv)
{
	cerr << "usage: " << argv[0] << " [options] <latitude> <longitude>" << endl
		 ...
		 << "    -a, --category=CATEGORY       	filter with category" << endl
		;
}
...
parse_arguments(int & argc, char ** & argv)
{
	...
	{
		int optndx = 0;
		int opt = getopt_long(argc, argv, "uh:p:U:P:n:s:r:c:",
							  long_options, &optndx);
		...
		case 'c':
			g_filter_category = strdup(optarg);
			break;
